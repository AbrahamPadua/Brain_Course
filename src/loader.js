const loader = (canvas) => {
  ...
  class Electron {
    ...
    getPosition() {
      ...
      x =
        (this.synapse.to.x + toOffset * Math.cos(direction)) * (1 - progress) +
        (this.synapse.from.x - fromOffset * Math.cos(direction)) * progress;
      y =
        (this.synapse.to.y + toOffset * Math.sin(direction)) * (1 - progress) +
  ...
  function draw() {
    ...
    for (let synapse of brain.synapses) {
      ...
      ctx.beginPath();
      ctx.moveTo(
        (synapse.from.x + perp.x * width) / 2,
        (synapse.from.y + perp.y * width) / 2
      );
      ctx.bezierCurveTo(
        (point1.x + perp.x * width) / 2,
        (point1.y + perp.y * width) / 2,
        (point2.x + perp.x * widthMiddle) / 2,
        (point2.y + perp.y * widthMiddle) / 2,
        (middle.x + perp.x * widthMiddle) / 2,
        (middle.y + perp.y * widthMiddle) / 2
      );
      ctx.bezierCurveTo(
        (point3.x + perp.x * widthMiddle) / 2,
        (point3.y + perp.y * widthMiddle) / 2,
        (point4.x + perp.x * width) / 2,
        (point4.y + perp.y * width) / 2,
        (synapse.to.x + perp.x * width) / 2,
        (synapse.to.y + perp.y * width) / 2
      );
      ctx.lineTo(
        (synapse.to.x - perp.x * width) / 2,
        (synapse.to.y - perp.y * width) / 2
    ...
    for (let electron of brain.electrons) {
      ...
      ctx.beginPath();
      ctx.moveTo(upSide.x / 2, upSide.y / 2);
      ctx.arcTo(
        (upSide.x + Math.cos(direction) * electron.size) / 2,
        (upSide.y + Math.sin(direction) * electron.size) / 2,
        fontSide.x / 2,
        fontSide.y / 2,
        electron.size / 2
      );
      ctx.arcTo(
        (downSide.x + Math.cos(direction) * electron.size) / 2,
        (downSide.y + Math.sin(direction) * electron.size) / 2,
        downSide.x / 2,
        downSide.y / 2,
        electron.size / 2
      );
      ...
      ctx.bezierCurveTo(
        (downSide.x - Math.cos(direction) * electron.size) / 2,
        (downSide.y - Math.sin(direction) * electron.size) / 2,
        (backSide.x + Math.cos(direction) * electron.size) / 2,
        (backSide.y + Math.sin(direction) * electron.size) / 2,
        backSide.x / 2,
        backSide.y / 2
      );
      ctx.bezierCurveTo(
    ...
    for (let neuron of brain.neurons) {
      ...
      ctx.beginPath();
      ctx.arc(neuron.x / 2, neuron.y / 2, neuron.size / 2, 0, 2 * Math.PI);
